...
export function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-4 md:px-6 py-0 flex items-center justify-between gap-4 md:gap-8 relative">
          <Link href="/" className="flex items-center group flex-shrink-0 h-16 md:h-20 overflow-hidden max-w-[60%] lg:max-w-none">
          ...
          {/* Navigation Links - Desktop - Hidden on mobile (max-width: 767px), visible on md and up (>= 768px) */}
          ...
          {/* Mobile Menu Button - Only visible on mobile (max-width: 767px) */}
          <div className="flex items-center z-50 relative md:hidden">
            <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
              <SheetTrigger asChild>
                <button
                  className="p-2.5 text-foreground hover:text-primary transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 rounded-md flex-shrink-0 min-w-[44px] min-h-[44px] flex items-center justify-center"
                  aria-label="Open menu"
                  type="button"
                >
                  <Menu className="h-7 w-7 stroke-[2.5] stroke-current" />
                </button>
              </SheetTrigger>
          ...
          {/* Social Icons - Desktop Only - Hidden on mobile (max-width: 767px), visible on md and up (>= 768px) */}
      ...
      <div className="bg-foreground text-background py-3 md:py-2">
        <div className="container mx-auto px-4 md:px-6 grid grid-cols-1 md:grid-cols-3 items-center gap-4 md:gap-4">
          {/* Phone Section - Centered on mobile, left-aligned on desktop */}
          <div className="flex items-center gap-3 justify-center md:justify-start">
            <div className="bg-primary rounded-full p-2.5 md:p-3 flex-shrink-0">
              <Phone className="h-4 w-4 md:h-5 md:w-5 text-primary-foreground" />
            </div>
            <div className="text-center md:text-left">
              <div className="text-base md:text-lg font-bold">082 549 6063 | 061 513 7249</div>
              <div className="text-xs text-background/70">Contact Us Anytime</div>
            </div>
          </div>

          {/* Open Hours Section - Centered on mobile, centered on desktop */}
          <div className="flex items-center gap-3 justify-center">
            <div className="bg-primary rounded-full p-2.5 md:p-3 flex-shrink-0">
              <Clock className="h-4 w-4 md:h-5 md:w-5 text-primary-foreground" />
            </div>
            <div className="text-center">
              <div className="text-base md:text-lg font-bold">Open Hours</div>
              <div className="text-xs text-background/70">Weekdays 8:00-18:00, Sat: Closed</div>
            </div>
          </div>

          {/* Schedule Button - Centered and responsive width on mobile, right-aligned on desktop */}
          <Link href="#contact" className="flex justify-center md:justify-end w-full md:w-auto">
            <Button className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-full px-6 py-5 md:px-8 md:py-6 text-sm md:text-base font-medium w-[90%] md:w-auto max-w-sm md:max-w-none">
              SCHEDULE A PICKUP
            </Button>
          </Link>
        </div>
